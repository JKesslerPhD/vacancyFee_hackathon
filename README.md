...
### Known gaps: park land, occupied structures, and public infrastructure slip into the vacant set

Three independent false-positive categories, all patched downstream by `hackathon_data/qc_vacancy_exclusions.py` (not fixed in `build_hackathon_data.py` itself, which isn't in this repo):
...
- **Occupied structures with missing improvement data.** `VAL_ASSD_IMPRV` is *null* — not recorded as literally $0 — for every single Tier 2 row; the classification treats missing data as zero. **1,290 parcels** (14% of Tier 2, plus a few Tier 3) show direct structural evidence (building square footage, bedroom count, or a year-built) that contradicts "zero improvement" — the largest category is coded `SINGLE FAMILY RESIDENTIAL` (646 of those), and hand-sampled examples include a 2017-built 4BR home that sold in 2022 and City of Sacramento Housing Authority-owned public housing built 2008. Tier 1 (land-use-code based, not improvement-value based) doesn't have this problem — only 2 of 19,295 show any structural evidence.
- **Inherently active public infrastructure the structural check still misses.** Tax-exempt government parcels frequently have *every* structural field null (private property gets appraised in more detail), so **113 more parcels** slipped past the check above: 55 of 56 parcels coded `PARKING GARAGE, PARKING STRUCTURE` are actual downtown Sacramento public parking garages (City/County/State/community-college-district owned, e.g. 1000 I St, 725 7th St); the rest are `AIRPORT & RELATED` (County of Sacramento, on streets literally named Earhart Dr / Lindbergh Dr — Sacramento Executive Airport) and `CEMETERY (EXEMPT)` (cemetery districts, the U.S. government, the Catholic diocese). Deliberately *not* extended to offices, retail, or golf courses — those legitimately can sit vacant/closed and are exactly what "vacant building" fee policy targets.

Run `qc_vacancy_exclusions.py` to regenerate `vacant_parcels_qc.csv`/`.geojson`; `311_heatmap/`, `maps/`, and `revenue_impact/` pick the corrected files up automatically when present. See `hackathon_data/qc_exclusion_report.csv` (tagged with an `exclusion_reason` column) for the full flagged list — 1,647 parcels total. These are patches on the output, not a fix to `build_hackathon_data.py` itself — that script should apply all three exclusions at the source.
