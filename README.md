...
| `hackathon_data/qc_vacancy_exclusions.py` | QC pass: flags parcels that are public park/recreation-agency owned, or coded zero-improvement despite structural evidence of an occupied building, that slipped into `vacant_parcels.csv`/`.geojson`; writes corrected `*_qc` exports plus `qc_exclusion_report.csv`. See "Known gaps" below. |
...
### Known gaps: park land, Tier 2's "zero improvement" heuristic, and parking lots

Three independent false-positive/ineligibility categories, all patched downstream by `hackathon_data/qc_vacancy_exclusions.py` (not fixed in `build_hackathon_data.py` itself, which isn't in this repo):

- **Public park land.** The "excluding ... parks" language above isn't actually applied upstream. Publicly-owned park land is tax-exempt, so it carries $0 improvement value and matches Tier 2 exactly like a genuinely vacant lot; undeveloped park/open-space parcels can also carry a Tier-1 "I" land use code. Owner-name-matching against known public park/recreation agencies (special recreation & park districts, CA State Parks) found **244 parcels** across 20+ agencies (Southgate, Cordova, Sunrise, Fair Oaks, North Highlands, Orangevale, CA Dept of Parks & Recreation, etc.) — 175 in Tier 2, 69 in Tier 1.
- **Tier 2 "zero improvement" mostly means "no improvement data," not "no building."** `VAL_ASSD_IMPRV` is *null* — not recorded as literally $0 — for every single Tier 2 row, county-wide, regardless of use type. Of the ~91 distinct use codes that show up in Tier 2/3, the overwhelming majority are specific building/business types — `OFFICE BLDG (MULTI-STORY)`, `APARTMENTS`, `HOTEL`, `RESTAURANT`, `MEDICAL BLDG/CLINIC`, `WAREHOUSE`, `GAS STATION`, `GOLF COURSE`, dozens more — not "vacant land" by any description. (This is also why a narrower fix that only checked for stray structural fields like `BUILDING_SQFT` wasn't enough on its own: tax-exempt government parcels routinely have *every* field null too — 55 of 56 parcels coded `PARKING GARAGE, PARKING STRUCTURE` turned out to be actual downtown Sacramento public garages, e.g. 1000 I St, with zero structural fields captured at all.) The fix keeps an explicit allowlist of the use codes that actually mean "no structure" (`RESIDENTIAL/COMMERCIAL/INDUSTRIAL-VACANT LAND`, `WASTE LAND, MARSH, SWAMP...`, `QUARRIES`, etc. — see `VACANT_LAND_USE_CODES` in the script) and treats everything else in Tier 2/3 as miscoded — **3,808 parcels**. Tier 1 (land-use-code based, not improvement-value based) doesn't have this problem and isn't touched — only 2 of 19,295 Tier 1 parcels show any structural evidence in the first place.
- **Parking lots aren't a vacancy-tax target.** An operating surface lot is paved, in-use commercial property, not vacant land — **541 parcels** coded `PARKING LOT` (392 in Tier 2, 149 in Tier 3) are excluded regardless of tier. This empties Tier 3 ("Parking/Abandoned") almost entirely: it turns out to be 100% parking lots (BFH-coded) in this dataset, with zero abandoned service stations (BFK) actually present despite the tier's name.

Run `qc_vacancy_exclusions.py` to regenerate `vacant_parcels_qc.csv`/`.geojson`; `311_heatmap/`, `maps/`, and `revenue_impact/` pick the corrected files up automatically when present. See `hackathon_data/qc_exclusion_report.csv` (tagged with an `exclusion_reason` column) for the full flagged list — 4,593 parcels total. These are patches on the output, not a fix to `build_hackathon_data.py` itself — that script should apply all three exclusions at the source.
